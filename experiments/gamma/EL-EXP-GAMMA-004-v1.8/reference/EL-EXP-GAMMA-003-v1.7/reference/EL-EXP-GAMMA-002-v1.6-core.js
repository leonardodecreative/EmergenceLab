...
(function exposeGammaExperiment(root, factory) {
  ...
  const BUILD_ID = 'EL-EXP-GAMMA-002-v1.6.0-candidate';
  const PROTOCOL_ID = 'circular-phase-retrieval-repair';
  ...
  const CIRCULAR_PHASE_INDICES = Object.freeze([10, 11, 12, 33, 34, 35]);
  const CIRCULAR_PHASE_INDEX_SET = new Set(CIRCULAR_PHASE_INDICES);
  ...
  function phaseCoordinateDistance(a, b) {
    const linear = Math.abs(a - b);
    return Math.min(linear, 1 - linear);
  }

  function familyBalancedDistance(a, b, families, circularPhase = false) {
    if (!Array.isArray(a) || !Array.isArray(b) || a.length !== b.length) return Infinity;
    let familyTotal = 0;
    families.forEach((family) => {
      let squared = 0;
      for (let index = family.start; index < family.end; index += 1) {
        const difference = circularPhase && CIRCULAR_PHASE_INDEX_SET.has(index)
          ? phaseCoordinateDistance(a[index], b[index])
  ...
  function kernelPrediction(entries, querySignature, options) {
    ...
    eligible.forEach((entry, entryIndex) => {
      ...
      const distance = familyBalancedDistance(query, stored, families, options.circularPhase === true);
  ...
  function predictionsFor(archive, signature, event, config) {
    ...
    return {
      ...
      shuffledGammaRelationalCircular: kernelPrediction(entries, signature, {
        sigma: config.sigma,
        epsilon: config.epsilon,
        kind: event.kind,
        baselineMode: 'type',
      ...
      gammaRelationalCircular: kernelPrediction(entries, signature, {
        sigma: config.sigma,
        epsilon: config.epsilon,
        kind: event.kind,
        baselineMode: 'type',
  ...
  function makeAccumulators() {
    return {
      ...
      shuffledGammaRelationalCircular: new MetricAccumulator(),
      gammaRelationalCircular: new MetricAccumulator(),
  ...
  function scoreSeed(modelMetrics, config) {
    ...
    const linearGammaRmse = modelMetrics.gammaRelational.balancedRmse;
    const circularGammaRmse = modelMetrics.gammaRelationalCircular.balancedRmse;
    const shuffledCircularRmse = modelMetrics.shuffledGammaRelationalCircular.balancedRmse;
    const baselineImprovement = bestBaseline.rmse > 0
      ? (bestBaseline.rmse - circularGammaRmse) / bestBaseline.rmse
      : 0;
    const shuffledImprovement = shuffledCircularRmse > 0
      ? (shuffledCircularRmse - circularGammaRmse) / shuffledCircularRmse
      : 0;
    const linearImprovement = linearGammaRmse > 0
      ? (linearGammaRmse - circularGammaRmse) / linearGammaRmse
      : 0;
    ...
    schedule.forEach((event, eventIndex) => {
      ...
      if (event.phase === 'test') {
        ...
        testRecords.push({
          ...
          linearGammaPredictionMagnitude: vectorMagnitude(predictions.gammaRelational),
          circularGammaPredictionMagnitude: vectorMagnitude(predictions.gammaRelationalCircular)
        });
      }
  ...
  function runSuite(options = {}) {
    ...
    const result = {
      ...
      decision: {
        ...
        verdict: !valid
          ? 'INVALID_RUN'
          : 'PENDING_EXACT_SIGN_TEST_AND_INDEPENDENT_REPLICATION',
  ...
  return Object.freeze({
    ...
    contextVector,
    eventSignature,
    phaseCoordinateDistance,
    familyBalancedDistance,
    measureEvent,
    scheduleEvents,
    vectorMagnitude,
    predictionsFor,
